# Nikolai 0.3 â€?Technical Architecture
...
- **Node Coordinator:** Manages the registry and gRPC routing for external Shopper Assistant Nodes (AISLES).
...
To ensure the system remains responsive, user input is handled by a non-blocking loop that utilizes a thread executor for `stdin` reads. This allows the system to continue processing AISLES telemetry and background tasks while waiting for user input.

---

## 3. Integration with Shopper Assistant Node (AISLES)
...
4. **Node Discovery:** Silica AISLES Bridge opens the gRPC port on the Tailnet IP.
