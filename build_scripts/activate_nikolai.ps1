# ---------------------------------------------------------
# Nikolai 0.3 – Controlled Activation Script (Fresh Build)
...
if (-not (Test-Path $verifyScript)) {
    Write-Host "❌ Missing verify_build.ps1 — cannot activate."
    exit 1
}
...
if ($verify.ExitCode -ne 0) {
    Write-Host ""
    Write-Host "❌ Build verification failed — activation aborted."
    exit 1
}
...
if ($process.ExitCode -ne 0) {
    Write-Host ""
    Write-Host "❌ Activation failed — runtime error."
    exit 1
}
