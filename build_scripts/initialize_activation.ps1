# ---------------------------------------------------------
# Nikolai 0.3 – Activation Preparation (Fresh Build)
...
try {
    ...
    Write-Host "✔ Spine JSON valid."
}
catch {
    Write-Host "❌ Spine JSON invalid — activation cannot proceed."
    exit 1
}
...
if ($process.ExitCode -ne 0) {
    Write-Host ""
    Write-Host "❌ Reflex tests failed — activation blocked."
    exit 1
}

Write-Host "✔ Reflex tests passed."
Write-Host ""
...
if ($process.ExitCode -ne 0) {
    Write-Host ""
    Write-Host "❌ Runtime failed to load — activation blocked."
    exit 1
}

Write-Host "✔ Runtime loads cleanly."
Write-Host ""
